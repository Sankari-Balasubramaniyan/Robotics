...
'''
 we copied the content of train file in here.
  for some reason we cannot import it.
'''
import rospy
...
def talker() :
    pub = rospy.Publisher ('gesture', String, queue_size=1)
    rospy.init_node('gesture_publisher', anonymous=True)
    clf = create_trained_clf( 'data.csv')
  
    while not rospy.is_shutdown():
        gesture_str = capture_gesture(clf, 'tem_file.avi', 150) 
        rospy.loginfo(gesture_str[0])
        pub.publish(gesture_str[0])
